    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance <= 2.5 then
            QBCore.Functions.TriggerCallback('police:server:getCuffStatus', function(cuffstat)
                ...
                TriggerServerEvent('InteractSound_SV:PlayWithinDistanceOnCoords', 3.0, "Cuff", 0.2, GetEntityCoords(PlayerPedId()))
...
RegisterNetEvent('police:client:GetCuffed', function(source, position, item)
    ...
    if not isHandcuffed then
        ...
        if not cuffed and Config.CuffItems[item] then
            TriggerServerEvent('police:server:AddRemove', Config.CuffItems[item].itemname, 1, "remove", source)
            if Config.CuffItems[item].needkey then TriggerServerEvent('police:server:AddRemove', Config.CuffKeyItem, 1, "add", source) end
        end
        isHandcuffed = true
        ...
        if PlayerFreezed and Config.CuffItems[item].itemname == "handcuffs" then
        ...
        if tostring(item) == Config.CuffKeyItem or tostring(item) == Config.CutTieItem then
            ...
            TriggerServerEvent('InteractSound_SV:PlayWithinDistanceOnCoords', 3.0, "Uncuff", 0.2, GetEntityCoords(PlayerPedId()))
...
RegisterNetEvent('police:client:GetUnCuffed', function(item)
    ...
    ClearPedTasksImmediately(ped)
    -- TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
    TriggerServerEvent('InteractSound_SV:PlayWithinDistanceOnCoords', 3.0, "Uncuff", 0.2, GetEntityCoords(ped))
    QBCore.Functions.Notify(Lang:t("success.uncuffed"),"success")
end)
